...
class SilencePattern:
    ...
    # Generative
    common_reframes: List[str]             # alternative interpretations this pattern supports
...
class ShadowCatalog:
    ...
    def match_cues(self, text: str) -> List[SilencePattern]:
        """
        Pattern-match against a text (study description, claim, etc.)
        Returns patterns whose cues appear in the text.
        """
    ...
    def diagnose(self, study_description: str) -> str:
        """
        Run the catalog against a study description and return
        a diagnostic report of likely silence patterns.
        """
        matches = self.match_cues(study_description)
        if not matches:
            return "No cataloged silence patterns matched. Proceed to custom analysis."
