...
class ScopeMap:
    ...
    def as_liberation_statement(self) -> str:
        ...
        lines = [
            ...
            "=" * 70,
            "",
            f"WHAT THIS STUDY LOAD-BEARINGLY SHOWS:",
        ...
        lines.append("")
        lines.append(f"WHEN IT GOES SILENT:")
        for condition in self.silent_when:
            lines.append(f"  - {condition}")

        lines.append("")
        lines.append(f"QUESTIONS THIS STUDY CANNOT ANSWER:")
...
class ScopeMapper:
    ...
    def map_study(self,
        ...
        # Step 1: identify measurement type from instrument
        measurement_type = self._classify_measurement(measurement_instrument, what_was_measured)

        # Step 2: identify selection method from population description
        selection = self._classify_selection(population)

        # Step 3: restate claim with scope attached
        scoped_claim = self._restate_with_scope(
            claimed_finding, what_was_measured, population, environment, duration
        )

        # Step 4: derive what the study is SILENT on
        silent_on = self._derive_silences(
            claimed_finding, what_was_measured, population, environment, duration, uncontrolled
        )

        # Step 5: derive application boundaries
        applies_when = self._derive_applies_when(population, environment, duration, controlled)
        silent_when = self._derive_silent_when(population, environment, duration, uncontrolled)
    ...
    def _classify_measurement(self, instrument: str, measured: str) -> MeasurementType:
        ...
        if any(w in combined for w in ["cortisol", "skin conductance", "heart rate", "bp", "eeg", "fmri"]):
        ...
        if any(w in combined for w in ["survey", "questionnaire", "self-report", "reported"]):
        ...
        if any(w in combined for w in ["followup", "follow-up", "longitudinal", "years later"]):
    ...
    def _classify_selection(self, population: str) -> SelectionType:
        ...
        if any(w in pop_lower for w in ["student", "undergraduate", "university", "convenience"]):
            return SelectionType.CONVENIENCE_SAMPLE
        if "random" in pop_lower:
            return SelectionType.RANDOM_SAMPLE
        if any(w in pop_lower for w in ["patient", "clinical", "diagnosed", "treatment"]):
        ...
        if any(w in pop_lower for w in ["veteran", "survivor", "first responder", "long-term"]):
            return SelectionType.SURVIVAL_SELECTED
        return SelectionType.UNDEFINED

    def _infer_units(self, instrument: str) -> str:
        instrument_lower = instrument.lower()
        if "cortisol" in instrument_lower: return "ug/dL or nmol/L"
        if "skin conductance" in instrument_lower: return "microsiemens"
        if "heart rate" in instrument_lower: return "bpm"
        if "reaction time" in instrument_lower: return "ms"
        if "score" in instrument_lower: return "points/scale"
        return "unspecified"
    ...
    def _restate_with_scope(self, claim: str, measured: str, pop: str, env: str, dur: str) -> str:
        """
    ...
    def _derive_applies_when(self, pop: str, env: str, dur: str, controlled: List[str]) -> List[str]:
        conditions = [
    ...
    def _derive_silent_when(self, pop: str, env: str, dur: str, uncontrolled: List[str]) -> List[str]:
        conditions = [
...
if __name__ == "__main__":
    # The childhood-trauma / threat-response study
    result = map_scope(
