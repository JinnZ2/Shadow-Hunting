...
**Liberate knowledge by mapping its scope. A study in scope is usable. A study out of scope is information atrophy.**

License: CC0

**Source:** Imported from [JinnZ2/Logic-Ferret](https://github.com/JinnZ2/Logic-Ferret)
(`knowledge/` folder). The upstream files had been saved in a markdown-rendered
state (smart quotes, bold-dunders, fenced class bodies); the import restored
ASCII syntax and re-indented the class bodies so the modules are importable as
`shadow_hunting.knowledge`.

**Quick start from Shadow-Hunting:**

```python
from shadow_hunting.knowledge import StudyInput, liberate
```
...
```python
from shadow_hunting.knowledge import ShadowCatalog
...
```python
from shadow_hunting.knowledge import UserContext, ContextRole, recontextualize_silences
...
```python
from shadow_hunting.knowledge import Navigator
