...
Core primitives:
Node     - any point in the analysis (scope item, edge, build, external context)
...
class Node:
    ...
    node_type: NodeType
    content: str
    source: str = ""                           # study, field observation, prior analysis
...
class Session:
    ...
    focus_stack: List[str] = field(default_factory=list)  # history of where you've been
    ...
    def link(self, from_id: str, to_id: str, link_type: LinkType, reason: str = "") -> None:
        if from_id not in self.nodes or to_id not in self.nodes:
            raise ValueError(f"Both nodes must exist. Have: {list(self.nodes.keys())}")
    ...
    def neighbors(self, node_id: str) -> List[tuple]:
        ...
        for link in self.links:
            if link.from_id == node_id:
                result.append(("->", link.to_id, link.link_type.value, link.reason))
            elif link.to_id == node_id:
                result.append(("<-", link.from_id, link.link_type.value, link.reason))
        return result
    ...
    def snapshot(self) -> str:
        ...
        for type_name, nodes in by_type.items():
            ...
            for n in nodes:
                ...
                status_marker = {"open": "o", "parked": "p", "explored": "x", "resolved": "v"}.get(n.status, "?")
                content_preview = n.content[:60] + ("..." if len(n.content) > 60 else "")
                lines.append(f"  {marker}[{status_marker}] {n.id}: {content_preview}")
            lines.append("")

        if self.current_node_id:
            lines.append(f"NEIGHBORS OF CURRENT ({self.current_node_id}):")
            for direction, other_id, link_type, reason in self.neighbors(self.current_node_id):
                lines.append(f"  {direction} {other_id} [{link_type}]: {reason or '(no reason given)'}")
            lines.append("")
...
class Navigator:
    ...
    def silence(self, content: str, from_node: Optional[str] = None, tags: Optional[List[str]] = None) -> str:
        """Add a detected silence (something the study doesn't measure)."""
        node = Node(
        ...
        if from_node:
            self.session.link(from_node, node_id, LinkType.OPENS, "scope boundary identified")
        return node_id

    def edge(self, question: str, from_node: Optional[str] = None, tags: Optional[List[str]] = None) -> str:
        ...
        node_id = self.session.add_node(node)
        if from_node:
            self.session.link(from_node, node_id, LinkType.OPENS, "edge question generated")
        return node_id

    def context(self, observation: str, from_node: Optional[str] = None, source: str = "") -> str:
        ...
        node_id = self.session.add_node(node)
        if from_node:
            self.session.link(from_node, node_id, LinkType.RECONTEXTUALIZES, "external context injected")
        return node_id
    ...
    def build(self, what_to_build: str, from_node: str) -> str:
        ...
        node_id = self.session.add_node(node)
        self.session.link(from_node, node_id, LinkType.BUILDS_ON, "buildable application")
        return node_id
...
if __name__ == "__main__":
    ...
    # Park a thread for later
    nav.session.park(e3, reason="requires different sample, different protocol — later")
