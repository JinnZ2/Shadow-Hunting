...
class ApplicationBuilder:
    ...
    def build(self,
        ...
        plan.legitimate_applications.extend(
            self._derive_legitimate_applications(
                claim, scope_population, scope_environment, scope_duration, what_was_measured
            )
        )

        # Misapplications to flag
        ...
        # Recombination
        plan.recombination_candidates = self._derive_recombinations(claim, what_was_measured)
    ...
    def _derive_legitimate_applications(self, claim, pop, env, dur, measured) -> List[LegitimateApplication]:
        apps = []

        # Research hypothesis generation (always legitimate)
        apps.append(LegitimateApplication(
            domain=ApplicationDomain.FURTHER_RESEARCH,
            description="Hypothesis generation for studies in populations outside this scope",
            design_constraint=(
        ...
        # Mechanism study (if physiological measurement)
        if any(w in measured.lower() for w in ["conductance", "cortisol", "heart", "eeg", "fmri"]):
            apps.append(LegitimateApplication(
                domain=ApplicationDomain.FURTHER_RESEARCH,
        ...
        # Design-time information (when finding informs tool/system design)
        apps.append(LegitimateApplication(
            domain=ApplicationDomain.DESIGN_CONSTRAINT,
            description="Input to design of environments matching the study's scope",
            design_constraint=(
        ...
        # Frame correction (reframing public understanding)
        if any(w in claim.lower() for w in ["deficit", "impair", "blunted", "abnormal", "disorder"]):
            apps.append(LegitimateApplication(
        ...
        # Field validation as explicit next step
        if "lab" in env.lower():
            apps.append(LegitimateApplication(
                domain=ApplicationDomain.FIELD_VALIDATION,
                description="Field validation protocol to extend the finding's applicability",
                design_constraint=(
    ...
    def _derive_misapplications(self, claim, pop, env, uncontrolled) -> List[Misapplication]:
        mis = []

        # Clinical/diagnostic overreach
        if any(w in claim.lower() for w in ["adversity", "trauma", "stress", "deficit", "blunted"]):
    ...
    def _derive_extensions(self, claim, measured, uncontrolled) -> List[str]:
        ...
        # Uncontrolled variable directedness
